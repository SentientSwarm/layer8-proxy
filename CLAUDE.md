...
.env.example                   # Required env vars (LOCKSMITH_VERSION=v2.5.0 default, ...)
```
...
LOCKSMITH_VERSION=v2.5.0 docker compose build locksmith
...
- **Image versioning**: locksmith container is pinned by `${LOCKSMITH_VERSION}` (default `v2.5.0` at v1.5.0 of this bundle; site repo's `.env` overrides). Bumping it is the ordinary upgrade path. Site repos pin `layer8_version=vX.Y.Z` in `site.cfg` against this repo's tag.
