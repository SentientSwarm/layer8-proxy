...
`main` is the working branch. Cut feature branches from `main`. Versioned via git tags. **Current: v1.7.0** (paired with agent-locksmith v2.7.0 — Kamiwaza MCP tools through locksmith).

Phase G (per-agent credential overrides + OAuth session labels) shipped in v1.1.0 / agent-locksmith v2.1.0. Phase G2 (codex `ChatGPT-Account-ID` injection) shipped in v1.2.0 / agent-locksmith v2.2.0. Phase G3 (codex Responses body fixup — `store: false`, `stream: true`, default `instructions`) shipped in v1.3.0 / agent-locksmith v2.3.0. Phase G4 (codex required-header injection — `OpenAI-Beta`, `originator`) shipped in v1.4.0 / agent-locksmith v2.4.0, closing the codex transparent-integration loop. Phase H (1Password Service Accounts + Environments substrate as opt-in source-of-truth for site-repo `.env`) shipped in v1.5.0 / agent-locksmith v2.5.0; substrate scripts live under `examples/site/scripts/` (render, provision, migrate, lib/op-writeback) plus a best-effort 1P write-back hook in `rotate-operator-token.sh`. Substrate is opt-in — mode-0600 `.env` on disk remains a supported alternative. See `agent-locksmith/docs/user/operator/1password-setup.md` for the operator adoption guide. Phase I (agent connectivity — expose an agent as a `kind=model` registration) shipped in v1.6.0 / agent-locksmith v2.6.0; the layer8-proxy change was docs only (`docs/user/expose-an-agent.md` + the `add-a-tool.md` `--egress` section), while agent-locksmith's only code change was the codex seed-upstream `/backend-api/codex` suffix fix (catalog 2.2.0). agent-locksmith v2.6.0 also rolls up point releases v2.5.1 (personalized `/skill` catalog fix) and v2.5.2 (codex `max_output_tokens` strip). Design lives in `agents-stack/docs/spec/v0.4.0-agent-connectivity.md` + ADR-0007. Kamiwaza MCP tools through locksmith shipped in v1.7.0 / agent-locksmith v2.7.0: a registrar (`examples/site/scripts/kamiwaza-mcp-registrar.py`) discovers Kamiwaza-hosted MCP tools and registers them as ordinary `kind=tool` passthrough entries (so they inherit ACL/audit/egress/response-controls); agent-locksmith v2.7.0 adds `tls.upstream_ca_bundle` to verify Kamiwaza's private-CA MCP endpoints. MCP-client agents reach a tool at `${layer8_endpoint}/api/kamiwaza-<slug>/mcp`. See `docs/user/kamiwaza-mcp.md`. Validated live against `kamiwaza-dde`.
...
.env.example                   # Required env vars (LOCKSMITH_VERSION=v2.7.0 default, ...)
```
...
# Build the locksmith image with a specific version (defaults to v2.7.0).
LOCKSMITH_VERSION=v2.7.0 docker compose build locksmith
...
- **Image versioning**: locksmith container is pinned by `${LOCKSMITH_VERSION}` (default `v2.7.0` at v1.7.0 of this bundle; site repo's `.env` overrides). Bumping it is the ordinary upgrade path. Site repos pin `layer8_version=vX.Y.Z` in `site.cfg` against this repo's tag.
