...
`main` is the working branch. Cut feature branches from `main`. Versioned via git tags. **Current: v1.6.0** (paired with agent-locksmith v2.6.0 — Phase I agent connectivity).
...
.env.example                   # Required env vars (LOCKSMITH_VERSION=v2.6.0 default, ...)
```
...
# Build the locksmith image with a specific version (defaults to v2.6.0).
LOCKSMITH_VERSION=v2.6.0 docker compose build locksmith
...
- **Image versioning**: locksmith container is pinned by `${LOCKSMITH_VERSION}` (default `v2.6.0` at v1.6.0 of this bundle; site repo's `.env` overrides). Bumping it is the ordinary upgrade path. Site repos pin `layer8_version=vX.Y.Z` in `site.cfg` against this repo's tag.
